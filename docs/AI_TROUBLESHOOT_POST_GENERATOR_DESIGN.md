...
이 서비스는 GitHub Issue, Pull Request, Commit 기록을 분석해 **기술 트러블슈팅 블로그 초안**을 자동으로 생성한다.
목표는 기술 블로그를 쓸 때 **디버깅/문제 해결 과정을 문서화하는 데 드는 시간을 줄이는 것**이다.
시스템은 개발 히스토리를 분석해서 다음과 같은 구조의 글을 재구성한다.
...
### Agent 3 — Root Cause Detector (근본 원인 탐지)
...
### Agent 5 — SEO Generator (SEO 정보 생성)
...
3. GitHub를 1차 지식 소스(Single Source of Truth)로 사용
   - 실제로 일어난 일은 GitHub에 기록되어 있다고 보고, 거기서 최대한 많이 끌어옴
4. 단순 요약이 아닌, 개발 서사(Narrative) 재구성
   - “무슨 버그였고 어떻게 고쳤는지”의 시간적 흐름을 살리는 것에 집중
...
과거 Issue/PR/Commit 기록을 기반으로
읽을만한 기술 블로그 글을 자동으로 얻게 되고,
